...
const SeatSelection = () => {
    const { flightID, orderID } = useParams();
    ...
    // Thong tin ve theo orderID
    const fetchTickets = async (orderID) => {
        try {
            const response = await fetch(`${config.endpoint}/Tickets/order/${orderID}/tickets`);
            const data = await response.json();
            console.log("vé theo orderID : " + JSON.stringify(data, null, 2));
            console.log("So luong vé :" + tickets.length);
            setTickets(data);
        } catch (error) {
  ...
  //  Lay so ghe ra chia deu hang cot
    ...
    const confirmSeat = () => {
        ...
        // Lưu ghế vào session
        const sessionData = JSON.parse(sessionStorage.getItem('ticket')) || [];
        sessionData[selectedTicketsIndex] = {
            ...sessionData[selectedTicketsIndex],
            position_seat: seatValue,
        };
        sessionStorage.setItem('ticket', JSON.stringify(sessionData));
